...
| B | AI layer — ask the evidence | ✅ **DONE 2026-08-11** — BM25 over this repository's own markdown, no vector store. **A citation to a section the model was not shown rejects the whole answer** — as a 502 naming the fabricated anchors, not as the passages with a flag set (D27) |
| C | AI layer — Urdu briefs | ✅ **DONE 2026-08-11** — `?lang=ur` on `/simulate` and `/plan`, under the same two faithfulness guards the English narrator carries, and **503 rather than silent English** when it cannot be honoured (D27). Found and fixed a real defect in the process: `_NUMBER` counted the `2` in `km2` as a figure, which would have rejected every Urdu translation for correctly translating the unit |
| D | AI layer — real policy documents | ✅ **DONE 2026-08-11** — native-PDF extraction with a **verbatim-quote guard** against the document's own de-spaced text, and a coverage report that states how many measures the two levers *cannot* express. The one path in the project with no offline fallback, which it says out loud; it is a build step, not a route |
...
| D27 | The key is **required**, not optional — narrows D13 | **Reversed 2026-08-11.** The AI layer's three deterministic stand-ins are deleted: the agent's regex goal parser and lattice sweep, its template report, `/evidence/ask`'s retrieval-only answer, and `translate`'s English fallback. One reason covers all of them: **each was a different procedure returning the same response shape**, so a run that quietly became one reported a number the feature had not produced, under field names saying it had. A lattice sweep in rank order is not a search; the retrieved passages are not an answer with a flag set; English is not a translation. `/agent/search`, `/evidence/ask` and any `?lang=ur` request now answer **503 naming the environment variable**, and a provider dying mid-search arrives as an `error` event rather than a quieter result. **D13 is narrowed, not withdrawn**: the keys are still free-tier and need no card, so *zero budget* holds — what is gone is *zero configuration*. What deliberately did **not** change: `/simulate`, `/plan`, `/cube/*` and `/explain/spatial` still answer with no key, because their deterministic output is the product rather than a stand-in for it — `explain.py`'s templates and `explain_spatial.py`'s region table are the *source of truth* a model may only reword. `narrate` also keeps its silent fallback, and the asymmetry with `translate` is the test of the rule: rewording prose the reader can already read costs them nothing they asked for, and answering in the wrong language does |
| D25 | Where the model may be reached — **replaces D18** | **Four modules, and every call site carries a post-check on the model's own output.** `dsl/llm.py`, `agent/nodes.py`, `evidence/answer.py`, `policy/extract.py`, and nowhere else. D18's rule was "one file", which was the cheap way to enforce the real content — *the model's output is never trusted* — while there was one caller. A tool-calling search graph is not one file, and pretending otherwise buys either a 2,000-line `llm.py` or a rule everyone ignores. So the *enforceable* half is kept and the *convenient* half is dropped: **a new call site is a new decision, and a call site with no post-check is a bug.** The post-checks in place are `_numbers_are_faithful` + `_headline_figures_survive` (narrate, translate, describe_pattern), `Plan` re-validation + `dsl.validate.resolve` (planner, agent propose), `Objective` schema validation with a default fallback (agent parse_goal), citation resolution against the corpus (evidence), and a verbatim-quote match against the document's de-spaced text (policy). D13 is untouched: every one of these paths still answers with no key at all |
