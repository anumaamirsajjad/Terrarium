...
| 12 | Deployment | ⬜ |
| A | AI layer — the intervention search agent | ✅ **DONE 2026-08-11** — LangGraph loop over the real cores (D17 reopened on its own stated condition), a 121-block candidate lattice (D26), a greedy control every result is scored against, and `POST /agent/search` as SSE. **Requires a key** (D27): the goal parser, the proposer and the report were each a deterministic stand-in returning a search's shape without being one, and all three are deleted |
| B | AI layer — ask the evidence | ⛔ **WITHDRAWN 2026-08-11** (D29), hours after shipping. BM25 over this repository's own markdown worked, with a citation guard rejecting the whole answer over one fabricated anchor (502, not the passages with a flag set) — withdrawn at the product owner's request: a good question about the codebase, the wrong one about a specific run. `POST /simulate/chat` replaces it |
| C | AI layer — Urdu briefs | ⛔ **WITHDRAWN 2026-08-11** (D28), hours after shipping. `?lang=ur` on `/simulate` and `/plan` worked, under the same two faithfulness guards the English narrator carries, with **503 rather than silent English** when it could not be honoured (D27) — removed entirely at the product owner's request, not for a technical failure. In the process it had found and fixed a real defect: `_NUMBER` counted the `2` in `km2` as a figure, which would have rejected every Urdu translation for correctly translating the unit |
| D | AI layer — real policy documents | ⛔ **WITHDRAWN 2026-08-11** (D30), hours after shipping. Native-PDF extraction with a verbatim-quote guard against the document's own de-spaced text worked, and was the one path in the project with no offline fallback — removed entirely at the product owner's request: not needed, not a technical failure |
| E | AI layer — explain the map | ✅ **DONE 2026-08-11** — deterministic segmentation onto the same lattice, joined to the cube's own attributes and the equity core's decile cut, described by a model that may only reword the table |
| F | AI layer — talk about a result | ✅ **DONE 2026-08-11** (D29) — `POST /simulate/chat`, replacing the withdrawn Phase B. Multi-turn: the client resends the conversation's prior turns with each new question, and `dsl.llm.answer_result_question` grounds the answer in the `Brief` already computed for the current result, under the same `_numbers_are_faithful` guard `narrate` carries. No cube dependency; needs a key |

**Phases A–E are the plan in [AI_LAYER_PLAN.md](AI_LAYER_PLAN.md)**, built in its stated
order (A → C → B → E → D), then narrowed by **D27**: the optional-key constraint was lifted
on 2026-08-11 and the deterministic stand-ins were deleted rather than kept. `/simulate`,
`/plan`, `/cube/*` and `/explain/spatial` still answer with no key — their deterministic
output is the product, not a substitute for one.
...
| D17 | Agent framework | ~~**No LangGraph.**~~ **REOPENED 2026-08-10 for the search agent only, on this entry's own stated condition.** The sentence below — *"If agents later need to choose between interventions and iterate, that is a real graph and this reopens"* — is exactly what Phase A does: propose → check → run → score → decide, with an edge back to `propose` every time `dsl.validate.resolve` refuses. `agent/graph.py` is the only importer. **The planner's ruling is unchanged and still holds**: `dsl/planner.py` is two nodes over plain urllib and gets no graph runtime. Original: **No LangGraph.** Budgeted for in Phase 10 and not taken, for the reason D15 dropped `osmnx`: it is a graph runtime, and the planner is two nodes — parse, then validate — with no branching, no cycles and no shared state to checkpoint. `dsl/planner.py` is one function that tries a model and falls back to a regex parser. If agents later need to *choose* between interventions and iterate, that is a real graph and this reopens. **Partially reopened by D24 on 2026-08-10**: LangChain (not LangGraph) now carries the plain-language narrator. The reasoning above is unchanged for the *planner* — that path is still two nodes over plain urllib |
| D18 | Where the LLM may live | ~~**One module, `dsl/llm.py`, and nowhere else.**~~ **SUPERSEDED by D25 — widened to four modules on 2026-08-10.** The reasoning below is why the replacement is a *narrowing of scope with the same content*, not a relaxation: the rule that mattered was never the file count, it was that nothing downstream treats a model's output as a measurement. Original: **One module, `dsl/llm.py`, and nowhere else.** `ingest/` remains the only layer that fetches *data for the cube*; a planner call is a different thing — Layer 3, made on the user's behalf, with nothing downstream treating its output as a measurement. It is confined to one adapter for the same reason, and everything it returns is re-validated as a `Plan` before a core can see it. The key is optional: with none, `/plan` uses the rule parser and the rest of the layer never calls out at all |
...
| D23 | The fitted emission scale factor | **Recorded, not applied.** With the seasonal kernel (D21) the leave-one-station-out fit returns **scale ×11.9** on a correlation of **+0.53** — far more trustworthy than the ×3.49 it returned at +0.157, which is why this needed a decision rather than silence. It is still **not applied to anything the product ships**, because two confounds inflate it: the observations are *daily* medians against a ~10:30 model, so a diurnal offset partly lands in the slope rather than wholly in the intercept; and OpenStreetMap records **no brick kilns anywhere near Lahore** (0 in the tile, 0 in the wider region, checked), so a major winter source is missing from the modelled field and the fit absorbs it. What the slope does support is the plain statement that **the literature emission factors are probably low by close to an order of magnitude** — which is worth saying, and is different from silently multiplying every number on screen by twelve. Reopens if the inventory gains kilns or the comparison moves to overpass-hour observations. *The fit was run before the normalisation was decoupled from the plume; that change rescales the whole field by a uniform 1.2533x, which an affine fit absorbs exactly, so the correlation and the leave-one-out error are unchanged and only the slope moved — ×14.9 to ×11.9* |
| D27 | The key is **required**, not optional — narrows D13 | **Reversed 2026-08-11.** The AI layer's three deterministic stand-ins are deleted: the agent's regex goal parser and lattice sweep, its template report, and `/evidence/ask`'s retrieval-only answer. One reason covers all of them: **each was a different procedure returning the same response shape**, so a run that quietly became one reported a number the feature had not produced, under field names saying it had. A lattice sweep in rank order is not a search; the retrieved passages are not an answer with a flag set. `/agent/search` and (at the time) `/evidence/ask` answered **503 naming the environment variable**, and a provider dying mid-search arrives as an `error` event rather than a quieter result. **D13 is narrowed, not withdrawn**: the keys are still free-tier and need no card, so *zero budget* holds — what is gone is *zero configuration*. What deliberately did **not** change: `/simulate`, `/plan`, `/cube/*` and `/explain/spatial` still answer with no key, because their deterministic output is the product rather than a stand-in for it — `explain.py`'s templates and `explain_spatial.py`'s region table are the *source of truth* a model may only reword. `narrate` also keeps its silent fallback. *`translate` and `?lang=ur` were removed entirely by D28 the same day; `/evidence/ask` itself was withdrawn by D29, and `/simulate/chat` inherits the 503-with-no-key behaviour described here* |
| D25 | Where the model may be reached — **replaces D18, narrowed by D29/D30** | **`dsl/llm.py` and `agent/nodes.py`, and every call site carries a post-check on the model's own output.** Originally four modules — `dsl/llm.py`, `agent/nodes.py`, `evidence/answer.py`, `policy/extract.py` — of which the last two are gone, withdrawn with the features that used them (D29, D30). D18's rule was "one file", which was the cheap way to enforce the real content — *the model's output is never trusted* — while there was one caller. A tool-calling search graph is not one file, and pretending otherwise buys either a 2,000-line `llm.py` or a rule everyone ignores. So the *enforceable* half is kept and the *convenient* half is dropped: **a new call site is a new decision, and a call site with no post-check is a bug.** `/simulate/chat` (D29) is a new function inside `dsl/llm.py` rather than a new module, so the count does not grow with it. The post-checks in place are `_numbers_are_faithful` + `_headline_figures_survive` (narrate, describe_pattern, `answer_result_question`), `Plan` re-validation + `dsl.validate.resolve` (planner, agent propose), `Objective` schema validation with a default fallback (agent parse_goal). D13 is untouched: every one of these paths still answers with no key at all |
| D26 | How the agent proposes geometry — **preserves D6** | **The model never emits geometry. It selects from geometry the grid layer generated.** D6 keeps coordinates out of a `Plan`, and the naive reading of a tile-wide search — let the model emit polygon coordinates — is both a D6 violation and technically bad: a model asked for coordinates on a 201×202 grid hallucinates them and burns the step budget on polygons that select no cells. `api/candidates.py` computes a fixed 20×20-cell lattice from the cube (110 regions, each carrying its plantable canopy, population, emissions and mean LST) and the model returns `region_ids` plus plan levers. Regions are **mergeable**, so the agent can grow one rather than being stuck at 2 km granularity. The region ids live on the search state, never on the `Plan`, so a `Plan` still carries no geometry and remains applicable to whatever the user drew |
| D24 | The plain-language narrator | **LangChain, in `dsl/llm.py`, behind a guard that checks its arithmetic.** The dashboard's copy was written for somebody defending a number, not somebody deciding whether a plan is worth doing, so `explain.plain_summary` now writes a jargon-free version from templates and `llm.narrate` may reword it. Three constraints make a generative explainer defensible here when `brief_for` still refuses to be one: (a) it **rewords only** — it never sources a figure, reaches the cube, or decides what is worth saying; (b) `_numbers_are_faithful` is a **post-check on the model's own output**, not a prompt instruction, and any numeral absent from the template's version rejects the whole rewrite back to the template — rounding 16.7 to 17 counts as inventing a figure; (c) it **cannot raise**, so no key, no network, a refusal or bad JSON all return the template and `/simulate` keeps its shape. D18 survives intact: this is still one file. D17's *planner* reasoning is untouched — the framework earns its place only where one prompt has to reach two providers and the output is prose rather than a schema |
| D28 | Urdu support, withdrawn — reverses D20's "kept" clause | **Removed 2026-08-11**, at the product owner's request rather than a technical failure. Everything Urdu-specific comes out: the vocabulary tables and Eastern Arabic-Indic digit folding in `dsl/planner.py`, `dsl/llm.py`'s `translate`/`translate_lines`/`Language` and the digit fold they shared with the faithfulness guard, `?lang=ur` on `/plan` and `/simulate`, the frontend's language toggle and `dir="rtl"` handling, the self-hosted Nastaliq font, and the landing page's Urdu line in the command-palette demo. `narrate` is untouched — it was never language-specific, it only reworded English prose — and `_numbers_in`'s digit fold is dropped with it since nothing upstream can produce Eastern Arabic-Indic digits anymore. This reopens D20's stated reason for keeping the rule parser's Urdu vocabulary after voice capture was cut; that reasoning was sound at the time and is recorded here as superseded, not wrong |
| D29 | Ask the record, withdrawn — replaced by a result chat | **Removed 2026-08-11**, at the product owner's request rather than a technical failure — Phase B worked. `src/terrarium/evidence/` (corpus, retrieve, answer), `api/routes/evidence.py` and the frontend's `EvidencePanel` are deleted, along with `docs_root` from `Settings` (nothing else read it) and the `"evidence"` task preference in `dsl/llm.py`'s provider routing. In its place: `POST /simulate/chat`, a multi-turn follow-up over the *current result's* `Brief` rather than the repository's markdown — see the DSL section of `CLAUDE.md` for the guard it carries. The "explain this" link on the 2.5x hindcast figure (`App.tsx`'s `explainFigure`) now opens the chat pre-asked, instead of the record |
| D30 | Published policy documents, withdrawn | **Removed 2026-08-11**, at the product owner's request rather than a technical failure — Phase D worked, verbatim-quote guard and all. `src/terrarium/policy/` (extract, schema, store, to_plan, the committed fixture PDF), `api/routes/policy.py`, `api/schemas/policy.py`, `scripts/ingest_policy.py`, `scripts/extract_policy.py`, the frontend's `PolicyPanel`, `Settings.policy_dir`, the `"policy"` task preference in `dsl/llm.py`, and `GeminiAdapter.complete_json_with_pdf` (its only caller) are all deleted. `pypdf` is dropped from `pyproject.toml` — it had no other caller. `data/raw/policy/` and the DuckDB catalogue rows it produced are left on disk; nothing reads them, but nothing was asked to delete a local build artefact either |
| D31 | Costs, removed entirely | **Removed 2026-08-11**, at the product owner's request: not a display preference, a decision not to compute or reason about a dollar figure anywhere. `dsl/library.py` loses `estimate_cost`, `CostEstimate`, and the two literature unit constants; `dsl.validate.ResolvedPlan` and `api/schemas/plan.py`'s `PlanResponse` lose their `cost` field; `dsl/explain.py` drops every "Rough cost" / "Indicative cost" line from both the plain summary and the technical findings. The agent loses more than display: `Metric` drops `"cost_effectiveness"` and `Objective` drops `max_cost_usd`, so a goal can no longer state a budget — `agent/objective.py`'s `INFEASIBLE` sentinel becomes unreachable and is deleted with the constraint it guarded. `Outcome.cost_usd` and `SearchResult.cost_basis` (the latter added and removed the same day) are gone. The PM2.5 figure takes the cost figure's old place among `agent/nodes.py`'s **required** headline figures — a report could previously drop it silently; now, when a plan touches traffic, `REPORT_SYSTEM` requires the report to explain in plain words what the PM2.5 change means, not just state the number |
...
**Withdrawn by D28, 2026-08-11 — kept below as history, not as current behaviour.** Everything
this section describes (the rule-parser vocabulary, the digit fold, the voice capture it
was written for) has been removed from the codebase.
...
- **A small, deliberate vocabulary**: درخت, پودے, شجرکاری for planting; گاڑیوں, ٹریفک with
  ...
  which matters most, since winter is where the air result changes by 6-9x. Urdu also puts
